...
Table~\ref{table:calibration_result} shows the calibration results.  Note that $R_f$ is calculated using contracting-grid algorithm, therefore we cannot measure its standard deviation.  The most important angular measurement is $\theta_y$ because it accounts for the image rotation about the optical axis.  Errors in this parameter can result in sever artifacts in the reconstruction image.  Both $\theta_x$ and $\theta_z$ are less important, in fact these values were assumed to be zero in some calibration methods~\citep{Noo2000, Cho2005, Yang2006}.  
