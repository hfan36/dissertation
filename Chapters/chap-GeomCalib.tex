...
In order to solve Eq.~\ref{eq:mlem}, we have assumed that the noise on each pixel is independent and identically distributed Gaussian noise.  So $\mathrm{pr( \mathbf{g} | \mathbf{\theta} })$, is a collection of zero-mean Gaussian functions with identical variances.  Equation~\ref{eq:mlem} can be written equivalently as
%
\begin{equation}
\label{eq:ln_mlem}
\mathrm{\boldsymbol{\hat{\theta}}} = \arg\max_{\theta} \; \mathrm{ ln[ pr( \mathbf{g}|\boldsymbol{\theta}) ]}.
\end{equation}
The maximum-likelihood solution to a zero-mean Gaussian function is then reduced to solve
%
\begin{equation}
...
There are numerous search algorithms that can be used to search for the vector $\boldsymbol{\theta$} that resides in a multi-dimensional space.  We have opted to use the contracting-grid algorithm that allows identification of a function's minimum in a fixed number of iterations using a fixed grid size.  The grid used in the algorithm is the range of values that each parameter can take on.  The size of the grid is the number of points used for each parameter in each iteration.  Since we can physically measure each parameter in the beginning, at least within a few millimeters or degrees, the grid are centered about these measured initial values.  We have set the size for distance measurements to be $\pm 5$ mm and orientation measurements to be $\pm 3 \degree$.  We have chosen to set the number of grid points to be 4 in order to limit the amount of time used in each iteration. 
...
The spacing within each parameter grid for the k-th iteration is
%
\begin{equation}
