...
\begin{figure}[h]
	\begin{subfigure}[b]{0.2\linewidth}
		\centering
		\includegraphics[width = 4cm]{Bayer_pattern_on_sensor.eps}
		\caption{}
	...
	\begin{subfigure}[b]{0.65\linewidth}
		\centering
		\includegraphics[width = 9cm]{Absorption-X3.png}
		\caption{}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{microlens_array.pdf}
...
There is a similar advantage to CMOS detectors with respect to thermal noise, which has a variance that is proportional to bandwidth. The lower circuit bandwidth associated with parallel processing at the pixel level automatically results in lower noise.  The 1/f noise is further eliminated by CDS at the pixel level.  One way to see this is to note that if the power spectral density, $S(f)$, varies as $|f|^{-\beta}$, then its Fourier transform, the noise autocorrelation function, satisfies $R(\tau) \propto |\tau|^{\beta-1}$, which approaches a constant as $\beta \to 1$ (see \cite{Barrett2004}, Sec. 3.3.7).  As a result, low-noise, scientific-grade CCDs are often read out at only 50,000 pixels per second, while prosumer DSLRs can go over a thousand times faster.
...
\begin{figure*}[h]
	\begin{minipage}[b]{0.45\linewidth}
		\centering
		\includegraphics[width=7.5cm]{DQE_k.eps}
		\subcaption{}
	\end{minipage}
	\hspace{0.2cm}
	\begin{minipage}[b]{0.45\linewidth}
		\centering
		\includegraphics[width=7.5cm]{DQE_N.eps}
		\subcaption{}
	\end{minipage}						
...
The PIXIS 2048B is an ultrasensitive CCD camera, which has a large detector and pixel size compared to the Andor camera, and can cool down to -60$\degree$C in air.  The camera has a very slow acquisition speed of 100 kHz, which is the mode used for image acquisition.  The camera also has a higher readout speed at 2 MHz, which is used for adjusting the lens and field of view.  The quantum efficiency is over 95\% at $\mathrm{Gd_2O_2S:Tb}$'s emission wavelength. The D700 Nikon camera was described in the previous section.
%
\begin{figure}
	\begin{subfigure}[b]{0.45\linewidth}
	\centering
...
%\hline
%Camera Model & Pixel Pitch ($\mu$m) & Sensor Size (mm) & Detector Type & Minimum readout speed \\ \hline
%Andor Neo & 6.5 & 16.6 $\times$ 14.0 & Scientific CMOS & 1 \\ \hline
%Nikon D700 & 8.5 & 36 $\times$ 24 & CMOS & 1 \\ \hline
%PIXIS 2048B & 13.5 & 27.6 $\times$ 27.6 & CCD & 1 \\
%\hline
%\end{tabular}
...
%\small
%\begin{center}
%	\begin{tabular}{l | p{3cm} | p{3 cm} | p{3 cm} }
%	\hline
%	\hline
%	Cameras & Andor Neo & Princeton PIXIS 2048B & Nikon D700 \\
%	\hline
%	\hline
%	Sensor Type & scientific CMOS	& CCD & CMOS \\
%	\hline
%	Pixel Pitch & 6.5 $\mathrm{\mu m}$  & 13.5 $\mathrm{\mu m}$  & 8.5 $\mathrm{\mu m}$  \\
%	\hline
%	Array Size & 2560$\times$2160 & 2048$\times$2048 & 4256$\times$2832 \\
%	\hline
%	Imaging Area  & 16.6$\times$14 ($\mathrm{mm^2}$) & 27.6$\times$27.6 ($\mathrm{mm^2}$)  & 36$\times$24 ($\mathrm{mm^2}$)  \\
%	\hline
%	Readout Speed & 100 MHz & 100 kHz & 60 MHz \\
%	\hline
...
\begin{figure}[h]
\centering
	\includegraphics[scale=1]{ShutterSystemWLabel.pdf}
...
\includegraphics[width = 23cm]{Capture_labview_frontpanel.png}
