...
\caption{Common physical models for the iterative reconstruction algorithm. (a) The pencil beam model (b) The point-response model.}
\label{fig:physical_modeling}
\end{figure}
...
\noindent $\sum\limits_k h_{ik} \hat{f}_k^{(n)}$ takes the current object volume, $\hat{f}^{(n)}_k$, and calculates its forward projection image at all angles.  The forward projection images are then divided into the data image vector, $g_i$, for every angle.  The result, $\frac{g_i}{\sum\limits_k H_{ik} \hat{f}_k^{(n)}}$, is a ratio between a set of simulated projection images and experimental data.  This ratio is then used by the imaging matrix, $\sum\limits_i \, H_{ij}$, and back-projected into object space.  The next iteration for object voxels, $\hat{f}_j^{n+1}$, is calculated by multiplying the back-projected ratio by the current object volume, $\hat{f}_j^{(n)}$, and divide by a sensitivity function $\sum\limits_{i'} H_{i'j}$.  The sensitivity function, $\sum\limits_{i'} \, H_{i' j}$, is calculated by summing the $H$ matrix over all detector pixels, and it represents the contribution of the detector from all projection angles onto the object volume.

The most important factor in the MLEM algorithm is the imaging matrix, $H$, that models the imaging system.  The $H$ matrix is typically measured in PET or SPECT systems where a point source is scanned in the entire volume of the object and the signals are recorded for every object point at every detector pixel for all detectors.  This is impossible for CT because the $H$ matrix would have been astronomically large and impossible to store and use, so we have opted to model the CT system to compute the forward projection and backward projection process on-the-fly using the Graphics Processing Unit (GPU), instead of using a stored $H$ matrix.  
