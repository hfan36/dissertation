...
\noindent where $d_x$, $d_y$, and $d_z$ are the distances between the $x$, $y$, and $z$ planes, respectively.  They are also the lengths to the sides of the voxel.  We can calculate the initial and final intersection plane of the ray with the object volume using the plane indexes by first calculating a set of minimum and maximum parametric values for each ray using the following equation,
